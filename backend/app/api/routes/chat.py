...
def submit_feedback(feedback: FeedbackCreate, db: Session = Depends(get_db), current_user: Student = Depends(get_current_user)):
    ...
    # Optional check: does the message belong to a session owned by this user?
    # session = db.query(ChatSession).filter(ChatSession.id == chat_msg.session_id, ChatSession.student_id == current_user.id).first()
